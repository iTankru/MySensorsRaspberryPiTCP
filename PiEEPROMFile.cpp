...
#define EEPROMFILE "/tmp/eeprom.rom"
#define EEPROMSAVE "/var/lib/eeprom.rom"
...
void read_from_file() {
	int fd = open(EEPROMFILE, O_RDONLY, S_IRUSR);
	if (fd == -1) {
//		printf("file not found.\n");
		fd = open(EEPROMSAVE, O_RDONLY, S_IRUSR);
		if (fd == -1) return;
	}
	read(fd, _eeprom, EEPROM_SIZE);
	close (fd);

}

void write_to_file() {
	int fd1 = open(EEPROMFILE, O_CREAT | O_WRONLY, S_IRUSR);
	if (fd1 != -1) {
//		printf("file is created.\n");
	}
	write(fd1, _eeprom, EEPROM_SIZE);
	close (fd1);

}

void save_to_file() {
	int fd1 = open(EEPROMSAVE, O_CREAT | O_WRONLY, S_IRUSR);
	if (fd1 != -1) {
//		printf("file is created.\n");
	}
	write(fd1, _eeprom, EEPROM_SIZE);
